...
from models import *
from util import generate_vid_keys, get_output
...
def get_stats_for_all_vids(experiment = None, thresholds = None, metric = 'G_Mean', models_dir = None,\
  dset = 'Thermal', agg_type = None, raw = False, animate= False):
	...
	if not os.path.isfile(path):
		...
		init_videos(img_width = experiment.img_width, img_height = experiment.img_height, \
				raw = False, dset = dset)
		init_data_by_class(vid_class = experiment.vid_class, dset = dset,\
			raw = False, img_width = experiment.img_width, img_height = experiment.img_height)
	...
	with h5py.File(path, 'r') as hf:
		...
		for Fall_name in vid_dir_keys_Fall: # Fall_name in [Fall1, Fall2, ..., Fall35]

			if Fall_name == Fall_stop:
				# If we want to check only until particular video
				print('breaking at ', Fall_name)
				break

			# Get all Frames + Labels of a Fall video
