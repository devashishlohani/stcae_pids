...
if __name__ == "__main__":
    ...
    '''----Dummy Fall Detection (for demonstration)----'''
    learned_model = 'Models/Thermal_Dummy/DSTCAE_UpSamp.h5'
    #learned_model = 'Models/Thermal_Dummy/DSTCAE_Deconv.h5'
    #learned_model = 'Models/Thermal_Dummy/DSTCAE_C3D.h5'
    ...
    -> per-video not allowed for Intrusion detection because we have videos with only non-intrusion also.
       This helps test our model in only non-intrusion classes also. But this raises error more than 1 class is needed to calculate AUROC/AUPR
    -> Note: per_video should be used in case of animation of an intrusion or fall video'''
    ...
    if learned_model == None:
        ...
        dstcae_exp = SeqExp(pre_load=learned_model, dset=dset, win_len=win_len, img_width=img_width, img_height=img_height)
        print(dstcae_exp.model.summary())
        print(dstcae_exp.model.optimizer.get_config())
        dstcae_exp.test(eval_type=evaluation_type, RE_type=RE, animate=do_animate, indicative_threshold_animation=use_indicative_threshold)
